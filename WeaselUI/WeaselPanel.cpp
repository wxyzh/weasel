...
#ifdef TEST
#ifdef _M_X64
	LOG(INFO) << std::format("From WeaselPanel::_ResizeWindow");
#endif // DEBUG
#endif // TEST
	CDCHandle dc = GetDC();
...
#ifdef TEST
#ifdef _M_X64
	LOG(INFO) << std::format("From WeaselPanel::_CreateLayout");
#endif // DEBUG
#endif // TEST
	if (m_layout != NULL)
		delete m_layout;
...
//更新界面
void WeaselPanel::Refresh(bool from_server)
{
...
#ifdef TEST
#ifdef _M_X64
	LOG(INFO) << std::format("From WeaselPanel::Refresh. hide_candidates = {}, margin_negative = {}, inline_no_candidates = {}", hide_candidates, margin_negative, inline_no_candidates);
#endif // DEBUG
#endif // TEST
...
bool WeaselPanel::_DrawPreedit(Text const& text, CDCHandle dc, CRect const& rc)
{
#ifdef TEST
#ifdef _M_X64
	LOG(INFO) << std::format("From WeaselPanel::_DrawPreedit. text = {}, rc.left = {}, rc.top = {}", to_string(text.str, CP_UTF8), rc.left, rc.top);
#endif // _M_X64
#endif // TEST
	bool drawn = false;
	std::wstring const& t = text.str;
	IDWriteTextFormat1* txtFormat = m_pDWR->pPreeditTextFormat;
	...
	if (!t.empty()) {
		...
		if (range.start < range.end) {
			...
			m_layout->GetTextSizeDW(after_str, after_str.length(), txtFormat, m_pDWR, &afterSz);
...
#ifdef TEST
#ifdef _M_X64
	LOG(INFO) << std::format("From WeaselPanel::_DrawPreeditBack. text = {}, rc.left = {}, rc.top = {}", to_string(text.str, CP_UTF8), rc.left, rc.top);
#endif // _M_X64
#endif // TEST
	bool drawn = false;
	std::wstring const& t = text.str;
...
#ifdef TEST
#ifdef _M_X64
					LOG(INFO) << std::format("From WeaselPanel::_DrawPreeditBack. m_layout->ShouldDisplayStatusIcon() = true");
#endif // _M_X64
#endif // TEST
...
bool WeaselPanel::_DrawCandidates(CDCHandle& dc, bool back)
{
#ifdef TEST
#ifdef _M_X64
	LOG(INFO) << std::format("From WeaselPanel::_DrawCandidates. back = {}", back);
#endif // DEBUG
#endif // TEST
	bool drawn = false;
	...
	IDWriteTextFormat1* txtFormat = m_pDWR->pTextFormat;
	IDWriteTextFormat1* labeltxtFormat = m_pDWR->pLabelTextFormat;
	IDWriteTextFormat1* commenttxtFormat = m_pDWR->pCommentTextFormat;
	...
	else
	{
		...
		for (auto i = 0; i < m_candidateCount && i < MAX_CANDIDATES_COUNT; ++i) {
			...
			if (!m_style.mark_text.empty() && COLORNOTTRANSPARENT(m_style.hilited_mark_color))
			{
				...
				_TextOut(hlRc, m_style.mark_text.c_str(), m_style.mark_text.length(), m_style.hilited_mark_color, m_pDWR->pTextFormat);
			}
			// Draw label
...
void WeaselPanel::DoPaint(CDCHandle dc)
{
	...
	if (!hide_candidates) {
		...
		if (m_layout->ShouldDisplayStatusIcon()) {
#ifdef TEST
#ifdef _M_X64
			LOG(INFO) << std::format("From WeaselPanel::DoPaint. m_layout->ShouldDisplayStatusIcon() = true");
#endif // _M_X64
#endif // TEST
			...
			/*((m_ctx.aux.str != L"🌕️" && m_ctx.aux.str != L"🌙️") ? m_iconEnabled : (m_status.full_shape ? m_iconFull : m_iconHalf)));*/
...
#ifdef TEST
#ifdef _M_X64
	LOG(INFO) << std::format("From WeaselPanel::_LayerUpdate. rc.left = {}, rc.top = {}, result = {}", rc.left, rc.top, result);
#endif // _M_X64
#endif // TEST
}
...
#ifdef TEST
#ifdef _M_X64
			LOG(INFO) << std::format("From WeaselPanel::MoveTo. Third: m_oinputPos.left = {}, from_rime = {}", m_oinputPos.left);
#endif // _M_X64
#endif // TEST
			m_inputPos = rc;
			m_oinputPos = m_inputPos;
...
#ifdef TEST
#ifdef _M_X64
	LOG(INFO) << std::format("From WeaselPanel::_RepositionWindow. adj = {}, m_inputPos.left = {}, m_inputPos.top = {}, m_inputPos.bottom = {}", adj, m_inputPos.left, m_inputPos.top, m_inputPos.bottom);
#endif // _M_X64
#endif // TEST
	RECT rcWorkArea;
...
void WeaselPanel::_TextOut(CRect const& rc, std::wstring_view psz, size_t cch, int inColor, IDWriteTextFormat* pTextFormat)
{
#ifdef TEST
#ifdef _M_X64
	LOG(INFO) << std::format("From WeaselPanel::_TextOut. rc.left = {}, rc.top = {}, psz = {}", rc.left, rc.top, to_string(psz.data(), CP_UTF8));
#endif // _M_X64
#endif // TEST
	...
	m_pDWR->m_pBrush->SetColor(D2D1::ColorF(r, g, b, alpha));

	if (NULL != m_pDWR->m_pBrush && NULL != pTextFormat) {
		m_pDWR->pDWFactory->CreateTextLayout(psz.data(), (UINT32)psz.size(), pTextFormat, (float)rc.Width(), (float)rc.Height(), reinterpret_cast<IDWriteTextLayout**>(&m_pDWR->pTextLayout));
		if (m_style.layout_type == UIStyle::LAYOUT_VERTICAL_TEXT) {
			...
			m_pDWR->pTextLayout->SetReadingDirection(DWRITE_READING_DIRECTION_TOP_TO_BOTTOM);
			m_pDWR->pTextLayout->SetFlowDirection(flow);
		}
		...
		m_pDWR->pTextLayout->GetOverhangMetrics(&omt);
		...
		if (m_pDWR->pTextLayout != NULL) {
			m_pDWR->pRenderTarget->DrawTextLayout({ offsetx, offsety }, m_pDWR->pTextLayout, m_pDWR->m_pBrush, D2D1_DRAW_TEXT_OPTIONS_ENABLE_COLOR_FONT);
#if 0
			D2D1_RECT_F rectf = D2D1::RectF(offsetx, offsety, offsetx + rc.Width(), offsety + rc.Height());
			pDWR->pRenderTarget->DrawRectangle(&rectf, m_pDWR->m_pBrush);
#endif
	}
		SafeRelease(&m_pDWR->pTextLayout);
}
}
